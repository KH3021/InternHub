...
export default function Login() {
  ...
  const [oauthLoading, setOauthLoading] = useState<'google' | 'github' | 'linkedin_oidc' | 'azure' | null>(null);
  ...
  const handleOAuthLogin = async (provider: 'google' | 'github' | 'linkedin_oidc' | 'azure') => {
    ...
    // Note: Do not clear loading state on success, as it will redirect
  };
      ...
      <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-800 text-center space-y-3">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <button
            ...
            className="py-2.5 px-3 rounded-xl border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 text-xs font-bold text-slate-700 dark:text-slate-200 transition-colors flex items-center justify-center gap-2 shadow-sm disabled:opacity-50"
            title="Sign in with Google"
          >
            {oauthLoading === 'google' ? (
              <div className="h-4 w-4 rounded-full border-2 border-primary-500/40 border-t-primary-600 animate-spin" />
            ) : (
