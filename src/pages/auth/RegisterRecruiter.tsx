...
import { Mail, Lock, User, Building, Phone, Eye, EyeOff, AlertCircle, CheckCircle2 } from 'lucide-react';
    ...
    return (
      <div className="max-w-md mx-auto w-full bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 p-8 rounded-3xl shadow-xl text-center space-y-4">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Confirmation link sent to <span className="font-bold text-slate-700 dark:text-slate-200">{email}</span>.
        </p>
    ...
    <div className="max-w-md mx-auto w-full bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 p-8 rounded-3xl shadow-xl transition-colors duration-300">
      <div className="text-center space-y-2 mb-6">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400">
          Start hiring from 50,000+ verified candidates.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4" autoComplete="off">
        ...
        <button type="submit" disabled={isSubmitting}
          className="w-full bg-gradient-to-r from-indigo-600 to-primary-600 hover:from-indigo-700 hover:to-primary-700 disabled:opacity-60 text-white font-bold text-sm py-3 rounded-xl shadow-lg transition-all flex items-center justify-center gap-2">
          {isSubmitting ? <div className="h-4 w-4 rounded-full border-2 border-white/40 border-t-white animate-spin" /> : null}
          <span>{isSubmitting ? 'Creating account…' : 'Create Recruiter Account'}</span>
        </button>
      </form>
