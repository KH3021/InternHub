...
import { Mail, Lock, User, Phone, Eye, EyeOff, AlertCircle, CheckCircle2, KeyRound } from 'lucide-react';
...
export default function RegisterCandidate() {
  ...
  const [regMode, setRegMode] = useState<'email' | 'mobile_otp'>('email');
  ...
  const handleSendMobileOtp = async () => {
    ...
    setIsSubmitting(true);
    const { error: err } = await sendOtp(phone, true);
    setIsSubmitting(false);
    if (err) {
      setError(err);
      return;
    ...
    setOtpSent(true);
    setInfoMsg(`OTP code sent to ${phone}. (Demo code: 123456)`);
  };
  ...
  const handleVerifyMobileOtp = async (e: React.FormEvent) => {
    ...
    setError('');
    setIsSubmitting(true);
    const { error: err } = await verifyOtp(phone, otpCode, true);
    setIsSubmitting(false);
    ...
    <div className="max-w-md mx-auto w-full bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 p-8 rounded-3xl shadow-xl transition-colors duration-300">
      ...
      <div className="grid grid-cols-2 gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl mb-4 text-xs font-bold">
        <button
          ...
          className={`py-2 rounded-lg transition-all ${regMode === 'email' ? 'bg-white dark:bg-slate-700 text-primary-600 dark:text-white shadow-sm' : 'text-slate-600 dark:text-slate-400'}`}
        >
          Email Registration
        </button>
        <button
          type="button"
          onClick={() => setRegMode('mobile_otp')}
          className={`py-2 rounded-lg transition-all ${regMode === 'mobile_otp' ? 'bg-white dark:bg-slate-700 text-primary-600 dark:text-white shadow-sm' : 'text-slate-600 dark:text-slate-400'}`}
        >
      ...
      {regMode === 'email' ? (
        ...
        <div className="space-y-4">
          ...
          {!otpSent ? (
            ...
            <form onSubmit={handleVerifyMobileOtp} className="space-y-4 animate-slide-up">
              <div>
                <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">Enter 6-Digit OTP Code</label>
                <input
                  type="text"
                  maxLength={6}
                  required
              ...
              >
                {isSubmitting ? <div className="h-4 w-4 rounded-full border-2 border-white/40 border-t-white animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}
